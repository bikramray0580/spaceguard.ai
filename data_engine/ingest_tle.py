...
def fetch_tle_data():
    ...
    request = Request(
        CELESTRAK_URL,
        headers={"User-Agent": "SpaceGuardAI/1.0"},
    )
...
def refresh_snapshot():
    """Refresh the checked-in snapshot from CelesTrak.

    The existing snapshot is never overwritten when download or validation
    fails. This makes the snapshot a safe offline fallback for demos.

    Returns True when fresh data was saved, False when the existing snapshot
    must be used instead.
    """
    print("Refreshing orbital data from CelesTrak...")
    try:
        ...
        save_json(objects)
        print(f"Live refresh successful: {len(objects)} current objects loaded.")
        return True
...
def main():
    if not refresh_snapshot():
        print("Data ingestion completed using fallback snapshot.")
    else:
